...
def get_menus_router(get_week_menu_manager: WeekMenuManagerDependency[ID],
    ...
    async def create_menu(request: Request, user_create: menu_create_schema,
        ...
        try:
            created_user = await menu_manager.create_menu(user_create, request=request)
    ...
    @router.get('', response_model=list[menu_read_schema], name='menus:get_menus_by_timestamp_range')
    async def get_menus_by_timestamp_range(menu_manager: BaseWeekMenuManager[ID] = Depends(get_week_menu_manager),
                                          start: int = 0, end: int = 9999999999, include_image: bool = False):
        return await menu_manager.get_menu_by_timestamp_range(start, end, include_image)
    ...
    async def update_menu(request: Request, id_or_checksum: str, user_update: WeekMenuUpdate,
        ...
        try:
            ...
            menu = await menu_manager.update_menu(user_update, current_menu, request=request)
    ...
    async def delete_menu(id_or_checksum: str, menu_manager: BaseWeekMenuManager[ID] = Depends(get_week_menu_manager),
        ...
        await menu_manager.delete_menu(menu)
    ...
    async def __get_menu_by(menu_manager: BaseWeekMenuManager[ID], id_or_checksum: str | None = None,
        ...
        try:
            if timestamp is not None:
                return await menu_manager.get_menu_by_timestamp(timestamp, include_image)
            elif re.search(r'^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$', id_or_checksum):
                return await menu_manager.get_menu_by_id(id_or_checksum, include_image)
            else:
                return await menu_manager.get_menu_by_checksum(id_or_checksum, include_image)
        except exceptions.WeekMenuNotExists as e:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=ErrorCode.MENU_NOT_FOUND) from e

    async def __get_image_by(menu_manager: BaseWeekMenuManager[ID], id_or_checksum: str) -> bytes:
        try:
        ...
        except exceptions.WeekMenuImageNotExists as e:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=ErrorCode.MENU_NOT_FOUND) from e
